...
public sealed class TweetContentParser
{
    ...
    public async Task<Result<TwitterContent, RequestVideoResult>> CreateTweetContentAsync(CancellationToken cancellationToken)
    {
        RequestVideoResult result;
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            ...
            return new(RequestVideoResult.Error);
        }

        if (result == RequestVideoResult.Success && _tweetContent.Media?.Count > 0)
        {
            return _tweetContent;
        }
        else if (result == RequestVideoResult.Success && _tweetContent.Media?.Count == 0)
        {
            return new(RequestVideoResult.NoVideo);
        }

        return new(result);
    }

    private async ValueTask<RequestVideoResult> TryInitializeTweetContentAsync(TweetIdentifier tweetIdentifier, CancellationToken cancellationToken)
    {
        ...
        if (_resultElement == null)
        {
            ...
            return RequestVideoResult.NotFound;
        }
        ...
        if (reason != null)
        {
            ...
            {
                "Suspended" => RequestVideoResult.AccountSuspended,
                "NsfwLoggedOut" => RequestVideoResult.AdultContent,
                "Protected" => RequestVideoResult.Protected,
                _ => RequestVideoResult.Error
            };
        }
        ...
        if (!tweetIdentifier.Id.Equals(restId, StringComparison.OrdinalIgnoreCase))
        {
            ...
            return RequestVideoResult.Error;
        }
        ...
        if (_legacyElement.TryGetProperty("extended_entities", out _extendedEntitiesElement))
        {
            ...
            return RequestVideoResult.Success;
        }
        ...
        if (_resultElement.Value.TryGetProperty("card", out _cardElement))
        {
            ...
            switch (tweetCardType)
            {
                ...
                case TwitterCardType.Broadcast:
                    ...
                    return RequestVideoResult.UnsupportedVideo;
                ...
                case TwitterCardType.VideoDirectMessage:
                    return await CreateFromCardWithVmapAsync("player_url", cancellationToken);
                default:
                    return RequestVideoResult.NoVideo;
            }
        }
    ...
    private async ValueTask<RequestVideoResult> CreateFromUnifiedCardAsync(CancellationToken cancellationToken)
    {
    ...
    private async ValueTask<RequestVideoResult> CreateFromCardWithVmapAsync(string vmapPropertyName, CancellationToken cancellationToken)
    {
        var bindingValues = _cardElement.GetPropertyOrNull("legacy")?.GetPropertyOrNull("binding_values");
        if (bindingValues == null)
        {
